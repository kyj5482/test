...
홈 상단에서 소개하는 앱 3개. 각 기능은 아래 퍼즐의 질문(이야기)에서 태어났다
(연결의 원천: `docs/content/products.json` — 글 하단 "이 이야기에서 태어난 기능"으로 역링크).

| 브랜드 | 문제 | 뿌리 퍼즐 |
|---|---|---|
| 🗄️ **SwimVault** | 리그·나라를 건널 때마다 끊기는 아이의 기록 | 퍼즐11 선수 성장 데이터 플랫폼 |
| ⏱️ **SplitLane** | 대회 결과는 남지만 매일의 훈련은 어디에도 안 남는다 | 퍼즐2 레이스는 디테일이다 · 퍼즐4b 챔피언의 코드 |
| 📋 **swim.meets** | psych sheet 수천 명 속에서 내 아이 찾기, 대기 5시간 | 퍼즐6 원정의 기술 |
...
mindmap
  root((콘텐츠))
    ...
    builder["💻 만드는 사람"]
      ...
      퍼즐11["🏊‍♂️ 선수 성장 데이터 플랫폼 (data·ai, 게시 예정 2026-08~10)"]
