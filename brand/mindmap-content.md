...
| `writer` | ✍️ **글 쓰는 아이의 곁** | 아이의 매일 글쓰기·AI 교육에 관심 있는 부모 | 온보딩 없음(항상 열림) |
  ...
  root((콘텐츠))
    ...
    builder["💻 만드는 사람"]
      ...
      퍼즐11["🏊‍♂️ 선수 성장 데이터 플랫폼 (data·ai) → SwimVault(swim.capsule)"]
        ...
        Q: AI는 기록 데이터에서 어떤 코칭을 뽑아낼 수 있을까?
        Q: 회사에서 만든 데이터 플랫폼 경험을, 개인 프로젝트로 옮길 수 있을까?
        Q: 내 아이를 위한 도구를, 다른 선수 가정도 쓰는 서비스로 만들 수 있을까?
      추천: dreamer 트랙 (실리콘밸리·주재원)
    writer["✍️ 글 쓰는 아이의 곁"]
      퍼즐12["✍️ 매일 쓰는 아이 (항상 열림) → tutor.writing"]
        Q: 아이의 글쓰기는 왜 매일 봐줄 사람이 없을까?
        Q: 빈 화면이 무섭지 않으려면 무엇을 먼저 보여줘야 할까?
        Q: 아이에게 쓸모 있는 피드백은 몇 개면 충분할까?
        Q: 같은 실수가 몇 주째 반복되는 걸 어떻게 잡을까?
        Q: 벌점 없이 계속하게 만드는 동기는 어떻게 설계할까?
        Q: 한 학기의 성장을 어떻게 눈으로 보게 할까?
      추천: builder 트랙 (AI 마이크로앱·데이터), parent 트랙
    dreamer["🌎 바다 건너를 꿈꾸는 사람"]
      퍼즐10["🌉 태평양 브릿지 (항상 열림, 8조각)"]
        Q: 2006년, 지도 없이 미국 20일을 어떻게 여행했나?
        Q: 밴쿠버 2개월, 여행과 살아보기는 무엇이 다를까?
        Q: 실리콘밸리 파견 2년은 무엇을 남겼나?
