# 서비스 배경 사진 크레딧 (Service background photo credits)

`art.js`의 서비스 배경으로 쓰는 다운로드 이미지의 출처·라이선스. 모두 Wikimedia Commons.
CC BY / CC BY-SA는 저작자 표시가 필요하므로 여기에 보존한다.
...
원본은 Wikimedia 썸네일 서비스로 폭 1280px로 받아 저장(전체화면 배경용). 교체 시 이 표도 함께 갱신.
